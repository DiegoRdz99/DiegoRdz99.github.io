...
return d}
function set_ctrl(po,s2,t){var i,p_v=s2.p_v,s={subtype:"midictl",p_v:p_v,v:p_v.v,chn:p_v.chn}
...
if(--w_instr==0)
play_start()})}
function def_instr(s,f,sf2par,sf2pre){var i
s=s.p_v.sym
while(s.ts_prev)
s=s.ts_prev
for(;s;s=s.ts_next){if(s.subtype!="midiprog")
continue
i=s.instr
...
po={conf:conf,onend:conf.onend||empty,onnote:conf.onnote||empty,s_end:i_end,s_cur:i_start,repv:i_lvl||0,tgen:2,get_time:get_time,midi_ctrl:midi_ctrl,midi_prog:midi_prog,note_run:note_run,timouts:[],v_c:[],c_i:[],ac:ac,gain:gain,rates:rates}
w_instr++
load_res(i_start)
if(--w_instr==0)
