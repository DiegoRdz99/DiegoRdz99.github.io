...
output+='"/>\n'}
function draw_slur(path,sl){var i,k,g,x1,y1,x2,y2,height,addy,a,y,z,h,dx,dy,ty=sl.ty,dir=(ty&0x07)==C.SL_ABOVE?1:-1,n=path.length,i1=0,i2=n-1,not1=sl.nts,k1=path[0],k2=path[i2]
...
break}
break}
if(tsnext&&s2.time>=tsnext.time){if(s2.grace&&s2.time==tsnext.time){for(s3=tsnext;s3&&s3.time==s2.time;s3=s3.ts_next){if(s3.type==C.GRACE)
break}
if(s3.type==C.GRACE)
s3=null}
if(!s2.grace||s2.time==tsnext.time||!s3){s.p_v.sls.push(sl);s2=s.p_v.s_next.prev
while(s2.next)
...
s=s.ts_next}
if(!s2.grace){while(s){if(s.v==v&&!s.invis)
path.push(s)
if(s==s2)
break
...
if(cache&&cache.st_l==stafflines&&cache.st_w==(w|0)){xygl(x1,staff_tb[st].y,'stdef'+cfmt.fullsvg)
return}
...
ln+='"/>'}
y=staff_tb[st].y
if(!cache&&w>get_lwidth()-10){cache={st_l:stafflines,st_w:w|0}
...
if(curvoice.tr_sco){i=nt_trans(note,i)
...
var strwh
(function(){if(typeof document!="undefined"){var el=abc2svg.eltxt
...
break}
for(;s;s=s.next){if((((!w_tb[s.type]&&s.type!=C.STAVES)||s.type==C.SPACE)&&s.time&&(!s.prev||s.time>s.prev.time+s.prev.dur))||(s.type==C.GRACE&&s.prev&&s.prev.type==C.GRACE)){s2={type:C.SM,v:s.v,p_v:s.p_v,time:s.time,dur:0,next:s,prev:s.prev,fmt:s.fmt}
if(s.prev)
s.prev.next=s2
else
voice_tb[s.v].sym=s2
...
nu=1
txt=""
for(s2=s;s2;s2=s2.next){if(s2.time>tim||s2.dur)
break
if(!s2.bar_type)
continue
if(s2.bar_type!='[')
...
map[2]=a[i]
break}}}
function set_transp(){var s
if(curvoice.ckey.k_bagpipe||curvoice.ckey.k_drum)
return
if(cfmt.transp&&curvoice.shift)
syntax(0,"Mix of old and new transposition syntaxes");if(is_voice_sig()){curvoice.okey.fmt=cfmt
curvoice.key=s=clone(curvoice.okey)}else{s=clone(curvoice.ckey)
s.k_old_sf=s.k_sf
sym_link(s)}
delete s.invis
key_transp(s)
...
break}
switch(param){case"start":self.block_gen({subtype:"mc_start"})
break
...
return
a_acc=[]
for(n=0;n<s.k_a_acc.length;n++){b40=abc2svg.pab40(s.k_a_acc[n].pit,s.k_a_acc[n].acc)
+curvoice.tr_sco
...
var s2=s.prev
if(!s.invis&&s2&&s2.type==C.BAR&&s2.bar_type[0]!=':'){s.next=s2
s.prev=s2.prev
if(s.prev)
s.prev.next=s
s2.prev=s
...
curvoice.eoln=true}}}
function get_key(parm){var v,p_voice,transp,sndtran,tr_p,nt,a=new_key(parm),s_key=a[0],s=s_key
a=a[1]
...
a=''}
goto_tune()}else{set_kv_parm(a)}
tr_p=curvoice.tr_p
curvoice.tr_p=0
if(s.k_sf==undefined){if(!s.k_a_acc&&!(tr_p&1))
return
...
delete p_voice.init
if(cfmt.transp&&parse.state>=2){p_v_sav=curvoice;curvoice=p_voice;curvoice.tr_sco=cfmt.transp
...
curvoice.clone.bol++}
set_kv_parm(a)
if(curvoice.tr_p){curvoice.tr_p=0
set_transp()}
v=curvoice.v
...
function gch_tr1(p){var i,o,n,a,ip,b40,tr=curvoice.tr_sco,csa=p.split('/')
...
w=an.text.wh[0]
if(an.pos==C.SL_ABOVE){y=y_get(s.st,true,s.x,w)+2
if(y>minmax[s.st].yup)
minmax[s.st].yup=y}else if(an.pos==C.SL_BELOW){y=y_get(s.st,false,s.x,w)-2
...
onerror(fn)}}
abc2svg.modules={ambitus:{},begingrid:{fn:'grid3'},beginps:{fn:'psvg'},break:{},capo:{},chordnames:{},clip:{},clairnote:{fn:'clair'},voicecombine:{fn:'combine'},diagram:{fn:'diag'},equalbars:{},gamelan:{},grid:{},grid2:{},jazzchord:{},jianpu:{},mdnn:{},MIDI:{},nns:{},pageheight:{fn:'page'},pedline:{},percmap:{fn:'perc'},roman:{},soloffs:{},sth:{},strtab:{},temperament:{fn:'temper'},tropt:{},nreq:0,hooks:[],g_hooks:[],load:function(file,relay,errmsg){function get_errmsg(){if(typeof user=='object'&&user.errmsg)
return user.errmsg
